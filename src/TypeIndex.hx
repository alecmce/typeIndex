...
class TypeIndex
{
    ...
    @:macro public static function getClassIDWithOptionalType(args:Array<Expr>):Expr
    {
        var type = args[args.length - 1];
        return macro TypeIndex.getClassID($type);
    }

    @:macro public static function getClassID(component:Expr):Expr
    {
        trace("Indexer.getClassID <<<");
        trace(component);
        trace(Context.typeof(component));
	...
	static function getTypeName(type:Type):String
	{
		...
		switch(type)
		{
			...
			case TMono(ref):
				var val = ref.get();
				if (val == null)
					Context.error("Cannot generate an ID for an untyped variable", Context.currentPos());
				else
					str.add(getTypeName(val));
			case TDynamic(t):
				Context.error("Cannot generate an ID for dynamic values", Context.currentPos());
